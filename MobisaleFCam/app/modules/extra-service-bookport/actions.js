...
/**
 * Reset data bookport (only object bookport)
 */
export function resetDataBookport() {
    return (dispatch) => {
        dispatch({ type: t.RESET_DATA_BOOKPORT});
    }
}
...
export function updateInfoRegistration(payload, callback) {
    return (dispatch) => {
        dispatch({type: t.UPDATE_INFO_REGISTRATION, payload });
        callback();
    }
}
