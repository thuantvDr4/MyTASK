    ...
    renderField(settings) {
        ...
        return (
            <View style={{height:40, flexDirection:'row', borderWidth:1, borderColor:"#D0D8E2", borderRadius:6, justifyContent:'center', alignItems:'center', paddingHorizontal:12}}>
                ...
                <View style={{flex:1}}>
                    {!selectedItem && <Text style={[styles.text, { fontSize:12, textAlign:'right', color: '#444444', paddingRight:20 }]}>{defaultText}</Text>}
                    {selectedItem && <Text style={[styles.text, { fontSize:12, fontWeight:'500', color: '#444444', textAlign:'right', paddingRight:20 }]}>{getLabel(selectedItem)}</Text>}
...
const styles = {
    ...
    ico: { position: 'absolute', right: 0, top: 0, marginTop: 3, width: 10, height: 10 },
}
