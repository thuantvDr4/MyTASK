/**
 * Màn hình danh sách hợp đồng để tạo bán thêm
 * @uthor anhpt94
 * @dateEdit ---
 */
...
const {updateInfoExtraServiceForm, updateBackScreenDetail} = customerInfo;
// API
import * as apiContractList from "../../contract-list/api";
import * as api from "../api";
import {getContractDetail} from "../../extra-service-infomation/api";
...
// COMPONENT
import PickerList from "../../contract-list/components/PickerList";
...
import styles from "../../contract-list/styles";

class ContractListExtraService extends React.Component {
    static navigationOptions = ({navigation}) => {
        return {
            title: strings("extra_service.contract_list.nav.titleCtl"),
            headerLeft: (
                <HeaderBackButton
                    onPress={() => NavigationService.navigateGoBack()}
                    tintColor="#fff"
                />
            ),
            headerRight: <View/>,
    ...
    listId = [
        {
            ...
            Phone1: "0987557633",
            Address:
                "3423 St.306, Sangkat Beoung Keng Kang 1, Khan 7Makara, Phnom Penh",
    ...
    constructor(props) {
        ...
        this.props.navigation.setParams({
            handleSelectMenuManage: this.handleSelectMenuManage.bind(this)
        });

        this.createCusInf = this.createCusInf.bind(this);
    }
    ...
    _handleLoadSearchType() {
        ...
        apiContractList.loadSearchType({}, (success, result, msg) => {
            this._loading(false);
    ...
    _handleLoadSearchDataAll() {
        ...
        const inputData = {
            Agent: this.state.dataSearch.SearchType,
            AgentName: this.state.dataSearch.SearchContent,
            LocationId:
                this.state.dataSearch.SearchLocation &&
                this.state.dataSearch.SearchLocation.Id
        };

        // load contract list để tạo bán thêm
        api.loadContractList(inputData, (success, result, msg) => {
            this._loading(false);
            if (success) {
                this.setState({
                    ...this.state,
    ...
    _handleChangeSearchType(value) {
        ...
        switch (value.value) {
            case 1:
                pl = strings("ctl.search.plhname");
                break;
            case 2:
                pl = strings("ctl.search.plhcon");
                break;
            case 3:
                pl = strings("ctl.search.plhphone");
                break;
            default:
                pl = strings("ctl.search.plhpp");
                break;
        }
    ...
    isValidData(temp) {
        ...
        if (
            dataSearch.SearchContent.replace(/^\s\s*/, "").replace(/\s+$/, "") === ""
        ) {
            errorList.push({
    ...
    /**
     * Navigate tới màn hình tạo customer info
     */
    createCusInf(objId) {
        if (objId) {
            //
            ...
            // get detail hợp đồng để tạo phiếu bán thêm
            getContractDetail({objId}, (success, result, msg) => {
                ...
                if (success) {
                    ...
                    if (result && result.length > 0) {
                        //update result lên store
                        this.props.updateInfoExtraServiceForm(result[0]);
                    }

                    //update lại màn hình back lại nếu tới bước detail hợp đồng mới tạo thành công
                    this.props.updateBackScreenDetail("ContractListExtraService");
    ...
    render() {
        ...
        var dataItem =
            !this.state.dataEmpty && Object.keys(data).length > 0 ? (
                <ScrollView
                    ...
                    {data
                        ? data.map((item, index) => (
                            ...
                            <View key={index} style={styles.oneList}>
                                <View style={styles.infoBox}>
                                    ...
                                    <View style={[styles.oneInfo, {marginTop: 8}]}>
                                        <Text style={styles.infoTitle}>
                                            {strings("ctl.item.lblStatus")}
                                        </Text>
                                        <Text style={[styles.infoValue, {color: "#45D91B"}]}>
                                            {item.ContractStatus}
                                        </Text>
                                    </View>

                                    {/*..V2.8..update...service type..*/}
                                    <View style={styles.oneInfo}>
                                        <Text style={styles.infoTitle}>
                                            {strings("ctl.item.lblServiceType")}
                                        </Text>
                                ...
                                {/*..footer..*/}
                                <TouchableOpacity onPress={() => this.createCusInf(item.ObjId)}>
                                    <View style={styles.createBox}>
                                        <View style={styles.btnCreate}>
                                            <Text
                                                style={styles.txtBtnCreate}>{strings("extra_service.contract_list.action.create_cus_inf")}</Text>
                                        </View>
                                    </View>
                                </TouchableOpacity>
                            </View>
                ...
                <View style={styles.dataEmpty}>
                    <View style={[styles.wrapImage]}>
                        <Image source={require("../../../assets/images/contract-list/report.png")}/>
                        <View>
                            <Text style={[ols.fs16, ols.fw500, {marginTop: 26, color: "#D6D6D6"}]}>
                                {strings("all.data.noDataContractList")}
                            </Text>
                        </View>
                    </View>
                </View>
            ...
            <View style={styles.container}>
				...
				*/}
                        ...
                        <View style={[styles.boxSearch, {width: "65%"}]}>
                            <View style={styles.viewInputSearch}>
                                <TouchableOpacity
                                    ...
                                    onPress={this._handleLoadSearchDataAll}
                                >
                                    <Image
                ...
                {/*
					Item loop
				*/}
                <View style={styles.listInfoContainer}>{dataItem}</View>
