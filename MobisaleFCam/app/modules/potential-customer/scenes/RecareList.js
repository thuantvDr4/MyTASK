...
*
* */
class ReCareList extends React.Component {

    static navigationOptions = ({navigation}) => {
        return {
            title: strings('potentianl_customer.reCare_list.title'),
    ...
    * */
    handleDefaultLoad =()=> {
        ...
        // Get API
        api.getRecareList_Bydate (myInput, (success, result, msg) => {
            // Hide loading
            this._loading(false);
...
* */
    ...
    * */
    submitSearch = () => {
        ...
        if(date1 > date2){
            this.refs['popup'].getWrappedInstance().show( strings('dl.potentianl_customer.recare_list.noti.date_not_match') );
        ...
        return (
            <View style={[i_styles.card_view]}>
                ...
                <View style={[i_styles.body_ctn]}>
                    <RowInfoIcon
                        // imageName={images.reCareIcon}
                        // iconStyle={{tintColor: '#A248FE'}}
                        mainStyle={{marginTop: 8}}
                        value={item.CustomerName}
                        label={strings('potentianl_customer.reCare_list.form.cusName')}/>

                    <RowInfo
                        mainStyle={{marginTop: 8}}
                        value={item.Phone}
                        label={strings('potentianl_customer.reCare_list.form.phone')}/>

                    <RowInfo
                        mainStyle={{marginTop: 8}}
                        value={item.Address}
                        label={strings('potentianl_customer.reCare_list.form.address')}/>
        ...
        return (
            <View>
                <View style={[i_styles.filterBar_st]}>
                    <Text style={i_styles.label_txt}>{strings('potentianl_customer.reCare_list.label_result')}</Text>
                </View>
        ...
        return (
            <View>
                <View style={i_styles.dateTime_ctn}>
                    <DateTimePicker
                        mainStyle={{height: 40,}}
                        label={strings('potentianl_customer.reCare_list.form.from_date_label')}
                        placeholder={strings(
                            'potentianl_customer.reCare_list.form.from_date_placeholder',
                        )}
                    ...
                    <DateTimePicker
                        mainStyle={{height: 40,}}
                        label={strings('potentianl_customer.reCare_list.form.to_date_label')}
                        placeholder={strings(
                            'potentianl_customer.reCare_list.form.to_date_placeholder',
                        )}
