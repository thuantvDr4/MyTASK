...
class SelectPicker extends React.Component {
    ...
    renderField(settings) {
        ...
        return (
            <View>
                {!selectedItem && <Text style={styles.txtPlaceholder}>{defaultText}</Text>}
                {selectedItem && <Text style={styles.txtPlaceholder}>{getLabel(selectedItem)}</Text>}
            </View>
        )
    }
...
const  styles = StyleSheet.create({
    ...
    txtPlaceholder : {
        color: '#444444', textAlign:'right', paddingRight:10, fontSize:12
    },
    exitHeaderSelect : {
        flex:1,
        textAlign:'center'
