 ...
 * @since Aug, 2018
 */
class PickerMultiSearchInput extends React.Component
{
    constructor(props)
    {
        super(props);
    ...
    mergeValueName(arrItem)
    {
        let name = [];
        for (i in arrItem) {
            name.push(arrItem[i].Name);
        }
        ...
        return (
            <View style = {[styles.wrapper, validStyle.validStyleBorder]}>
                ...
                <TouchableOpacity style={styles.fieldContainer} onPress={this.openOptionScreen.bind(this)}>
                    <Text style={[styles.textField, validStyle.validStyleText]}>{selectItem ? this.mergeValueName(selectItem) : this.props.placeholder} </Text>
...
PickerMultiSearchInput.defaultProps = {
    onChange: () => {}
};

export default PickerMultiSearchInput;
