 ...
 * 4. seletedText: Text display when selected
 ...
 * @since Aug, 2018
 */
class PickerCLKMInput extends React.PureComponent
{
    constructor(props)
    {
        super(props);
    ...
    componentDidMount()
    {
        if (this.state.selectItem) {
            setTimeout(() => {
                this.props.onChange && this.props.onChange(this.state.selectItem);
            }, 0);
    ...
    openOptionScreen()
    {
        NavigationService.navigate('SearchPickerCLKM', {
            ...
            <React.Fragment>
                <View style = {[styles.wrapper, validStyle.validStyleBorder]}>
                    ...
                    <TouchableOpacity style={[styles.fieldContainer]} onPress={this.openOptionScreen.bind(this)}>
                        <Text style={[styles.textField, validStyle.validStyleText]}>{selectItem ? this.props.seletedText : this.props.placeholder} </Text>
...
export default PickerCLKMInput;
